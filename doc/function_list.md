...
| 機能ID | 機能名 | 説明 | 状態 |
|-------|------|-----|------|
| AUTH-01 | メールアドレス認証サインアップ | メールアドレスとパスワードを使用してアカウント登録を行う | 実装済み |
| AUTH-02 | メールアドレス認証ログイン | 登録済みのメールアドレスとパスワードでログインする | 実装済み |
| AUTH-03 | Google認証ログイン | Googleアカウントを使用してログイン/登録する | 実装済み |
| AUTH-04 | ログアウト | 現在のセッションからログアウトする | 実装済み |
| AUTH-05 | パスワードリセット（リクエスト） | パスワードをリセットするためのリンクを含むメールを送信する | 実装済み |
| AUTH-06 | パスワードリセット（実行） | 新しいパスワードを設定する | 実装済み |
| AUTH-07 | メール確認 | 登録後にメールアドレスを確認する | 実装済み |
...
| POST-05 | 投稿詳細表示 | 投稿の詳細ページを表示する | 未実装 |
| POST-06 | 投稿編集 | 自分が作成した投稿を編集する | 未実装 |
| POST-07 | 投稿削除 | 自分が作成した投稿を削除する | 未実装 |
...
| UI-02 | ダークモード | ダークカラーテーマへの切り替え | 未実装 |
| UI-03 | 多言語対応（日本語） | 日本語インターフェースの提供 | 実装済み |
| UI-04 | 多言語対応（英語） | 英語インターフェースの提供 | 部分実装 |
...
| 機能ID | 機能名 | 説明 | 状態 |
|-------|------|-----|------|
| FUT-01 | コメント機能 | 投稿に対するコメントの追加・表示 | 未実装 |
| FUT-02 | 通知システム | 新しい投票やコメントの通知 | 未実装 |
| FUT-03 | ソーシャルシェア | SNSへの投稿共有機能 | 未実装 |
