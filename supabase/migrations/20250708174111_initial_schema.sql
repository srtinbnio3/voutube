...
CREATE TABLE IF NOT EXISTS "public"."channels" (
    ...
    "ownership_verification_method" "text" DEFAULT 'youtube_api'::"text",
    "last_ownership_check_at" timestamp with time zone,
    CONSTRAINT "channels_ownership_verification_method_check" CHECK (("ownership_verification_method" = ANY (ARRAY['youtube_api'::"text", 'manual'::"text"])))
);
...
-- =================================================
-- Auth Schema Related Setup (from auth_schema_queries.sql)
-- 認証関係のスキーマセットアップ
-- =================================================
...
-- ステップ2: プロフィール自動作成トリガーの作成（auth.usersテーブル用）
-- auth.usersテーブルに新規ユーザーが追加された時、自動的にprofilesテーブルにレコードを作成
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.create_profile_for_new_user();

-- 注意: このトリガーはauth.usersテーブル（認証スキーマ）に作用するため、
-- Supabase Branchingでは自動適用されない可能性があります。
-- プレビュー環境では手動で実行が必要な場合があります。
